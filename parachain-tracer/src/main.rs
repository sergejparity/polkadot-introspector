...
async fn main() -> color_eyre::Result<()> {
	...
	if opts.is_historical {
		...
		let mut historical_sub =
			HistoricalSubscription::new(vec![opts.node.clone()], from, to, opts.api_client_mode, opts.retry.clone());
		...
		let mut head_sub =
			ChainHeadSubscription::new(vec![opts.node.clone()], opts.api_client_mode, opts.retry.clone());
